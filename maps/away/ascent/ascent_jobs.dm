...
/mob/living/carbon/human/proc/queen_name()
	set name = "Name Queen"
	set category = "IC"
	set desc = "Rename yourself, your name should be \"Queen\", followed by an impressive sounding noun. For instance \"Queen Victory\""
	
	if(species.name == SPECIES_MONARCH_QUEEN && mind && istype(mind.assigned_job, /datum/job/submap/ascent))
		var/datum/job/submap/ascent/ascent_job = mind.assigned_job
		var/datum/submap/ascent/cutter = ascent_job.owner
		if(istype(cutter))
			var/new_name = sanitize(input("What is your name? your name should be \"Queen\", followed by an impressive sounding noun. For instance \"Queen Victory\"", "Name yourself") as text|null, MAX_NAME_LEN)
			if(!new_name)
				return
	...
	verbs -= /mob/living/carbon/human/proc/queen_name
...
/datum/job/submap/ascent/handle_variant_join(var/mob/living/carbon/human/H, var/alt_title)
	...
	switch(H.species.name)
		...
		if(SPECIES_MONARCH_QUEEN)
			H.real_name = "Monarch Serpentid Queen" // They will set their own name soon
			H.verbs |= /mob/living/carbon/human/proc/queen_name
